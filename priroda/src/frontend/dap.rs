...
use crate::debugger::{
    ExecutionResult, LocalDesc, PrirodaContext, SourceLocation, StackFrameDesc, StepResult,
};
...
impl<R: Read, W: Write> DapSession<R, W> {
    ...
    fn handle_continue<'tcx>(
        ...
        if session.has_pending_exception() {
            return Ok(HandlerSuccess {
                response: HandlerResponse::Error(
                    "program is stopped with an unrecoverable interpreter error; restart is not implemented yet"
                        .into(),
                ),
                state: None,
                events: Vec::new(),
