...
pub(super) struct PrirodaContext<'tcx> {
    ...
    // FIXME: add restart and other post-error commands, similar to GDB and
    // old Priroda, instead of only keeping the stored exception alive.
    pending_exception: Option<InterpErrorInfo<'tcx>>,
...
impl<'tcx> PrirodaContext<'tcx> {
    ...
    fn already_stopped_exception(&self) -> ExecutionResult {
        ExecutionResult::Stopped(StepResult::Exception {
            message: self.pending_exception_message().unwrap_or_else(|| {
                "the program has already hit an unrecoverable error".to_string()
            }),
        })
    }
    ...
    /// True when execution has hit an unrecoverable interpreter error.
    /// Execution commands keep reporting the stopped error until the
    /// debugger learns a restart or another explicit recovery command.
    pub(super) fn has_pending_exception(&self) -> bool {
        self.pending_exception.is_some()
    }

    pub(super) fn pending_exception_message(&self) -> Option<String> {
        self.pending_exception.as_ref().map(|err| err.kind().to_string())
    }
    ...
    fn resume(&mut self, mode: ResumeMode) -> InterpResult<'tcx, ExecutionResult> {
        loop {
            ...
            // interpreter errors as stopped debugger state until restart or
            // another recovery command is implemented.
